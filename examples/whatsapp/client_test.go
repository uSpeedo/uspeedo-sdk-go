...
import (
	...
	"github.com/uspeedo/uspeedo-sdk-go/services/whatsapp"
...
func getClient() *whatsapp.WhatsAppClient {
	cfg := uspeedo.NewConfig()
	credential := auth.NewCredential()
	credential.PublicKey = "PublicKey"
	credential.PrivateKey = "PrivateKey"
	return whatsapp.NewClient(&cfg, &credential)
}
...
func TestGet(t *testing.T) {
	...
	templates, err := c.GetTemplates(&apis.GetTemplatesReq{
	...
	sum, err := c.GetMessageSummary(&apis.GetMessageSummaryReq{})
	ass.Nil(err)
	ass.NotNil(sum)
...
func TestMedia(t *testing.T) {
	...
	umRes, err := c.UploadMedia(&apis.UploadMediaReq{
		BusinessPhone: phone.Number,
		File:          &fileString,
	})
	ass.Nil(err)
	...
	time.Sleep(100 * time.Millisecond)
	md, err := c.GetMedia(&apis.GetMediaReq{
		BusinessPhone: phone.Number,
		MediaId:       mediaId,
	})
	ass.Nil(err)
	...
	time.Sleep(100 * time.Millisecond)
	dr, err := c.DeleteMedia(&apis.DeleteMediaReq{
		BusinessPhone: phone.Number,
		MediaId:       mediaId,
	})
	ass.Nil(err)
...
func TestMessage(t *testing.T) {
	...
	msgType := "text"
	content := "none"
	msg, err := c.SendWhatsappMessage(&apis.SendWhatsappMessageReq{
		BusinessPhone: phone.Number,
		To:            &to,
		Type:          &msgType,
		Content:       &content,
