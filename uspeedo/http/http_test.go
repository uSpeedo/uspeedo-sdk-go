...
func TestHTTPRequestAccessor(t *testing.T) {
	...
	// query builder
	err = req.SetURL("https://api.uspeedo.com/api?foo2=bar")
	assert.NoError(t, err)
	assert.Equal(t, req.GetQueryMap()["foo2"], "bar")
	assert.Equal(t, req.GetURL(), "https://api.uspeedo.com/api")
	assert.Equal(t, req.GetQuery("foo2"), "bar")

	err = req.SetURL("https://api.uspeedo.com/api:12345?foo2=bar")
	assert.NoError(t, err)
	assert.Equal(t, req.GetQueryMap()["foo2"], "bar")
	assert.Equal(t, req.GetURL(), "https://api.uspeedo.com/api:12345")
	assert.Equal(t, req.GetQuery("foo2"), "bar")
}
...
func TestHTTPStatusError(t *testing.T) {
	...
	assert.Equal(t, "503 Service Unavailable", statusErr.Message)
}
